...
int main(int argc, char **argv, char **env)
{
  vluint64_t sample_time = 0;
	...
	Vsubservient_sim* top = new Vsubservient_sim;
	...
	while (!(done || Verilated::gotFinish())) {
	  ...
	  if (dump)
	    tfp->dump(main_time);
	  if (baud_rate)
	    do_uart(&uart_context, top->o_gpio);
	  else
	    do_gpio(&gpio_context, top->o_gpio);
